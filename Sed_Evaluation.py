...
def sed_event_eval(ref_file='./metadata/test/test_file_available_strong.csv', est_file='./submissions/630-0.28-threshold/sed_submission.csv'):
...
#         group.to_csv('./Sed_evaluation/'+os.path.splitext(key)[0]+'_ref.txt',index=False, header=False, sep='\t')
    ...
    for file_name in filelist:
        reference_event_list = sed_eval.io.load_event_list(
            filename='./Sed_evaluation/'+os.path.splitext(file_name)[0]+'_ref.txt', delimiter='\t', decimal='comma'
        )
        estimated_event_list = sed_eval.io.load_event_list(
            filename='./Sed_evaluation/'+os.path.splitext(file_name)[0]+'_est.txt', delimiter='\t', decimal='comma'
        )
