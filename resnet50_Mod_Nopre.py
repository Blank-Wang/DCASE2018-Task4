...
def ResNet50_Mod_Nopre(include_top=True, weights='imagenet',
    ...
    x = Conv2D(16, (3, 3), strides=(1, 2), padding='same', name='conv1')(img_input)
    x = BatchNormalization(axis=bn_axis, name='bn_conv1')(x)
    x = Activation('relu')(x)
    x = MaxPooling2D((3, 3), strides=(1, 2), padding='same')(x)

    x = conv_block(x, 3, [32, 32, 64], stage=2, block='a', strides=(1, 2))
    x = identity_block(x, 3, [32, 32, 64], stage=2, block='b')
    x = identity_block(x, 3, [32, 32, 64], stage=2, block='c')

    x = conv_block(x, 3, [64, 64, 128], stage=3, block='a', strides=(1, 2))
    ...
    x = conv_block(x, 3, [128, 128, 256], stage=4, block='a', strides=(1, 2))
    ...
    x = conv_block(x, 3, [128, 128, 256], stage=5, block='a', strides=(1, 2))
    x = identity_block(x, 3, [128, 128, 256], stage=5, block='b')
    x = identity_block(x, 3, [128, 128, 256], stage=5, block='c')

#     x = AveragePooling2D((7, 7), name='avg_pool')(x)
    x = AveragePooling2D((1, 1), padding='same', name='avg_pool')(x)
